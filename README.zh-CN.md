...
- **环境面板里「显示密钥值」的按钮，在默认侧栏宽度下看不见，而且能不能拖出来取决于窗口宽度。** 该按钮位于变量表的表头行，表头行不随内容横向滚动，所以它被侧栏边界裁掉之后**滚动是没有用的**——只能靠加宽侧栏。加宽后能否完整看到它，取决于窗口有多宽：
  - 窗口宽度 **≥ 约 1215px**：把侧栏拖到接近最右端，可以完整看到该按钮。
  - 窗口宽度 **约 1130–1215px**：拖到最右端只能得到一个被压扁的按钮（约 15–31px，能点，但明显不是完整尺寸）。
  - 窗口宽度 **< 约 1130px**（含 1024×768）：**无论怎么拖都看不到它**，在修复之前这个入口无法访问。

  同一张表里的「密钥」开关与「删除」按钮在默认宽度下同样在可视区外，但它们位于可横向滚动的列表区，**横向滚动就能碰到**。密钥值本身不受影响：请求发送时照常解析，受影响的只是「看一眼它是什么」这个只读入口。
