...
- [GitHub Releases](https://github.com/songzhibin97/ApiSolo/releases) 提供 macOS（Apple Silicon）的未签名 DMG 与 .app。
- 产物未做代码签名：首次打开请右键 → 打开，或执行 `xattr -dr com.apple.quarantine /Applications/ApiSolo.app`。
- 其他平台请按下方「开发」一节从源码构建。
...
- 从历史保存请求时，会在放行之前先列出那些已被脱敏的字段：每一项都标明来源（请求头、查询参数、表单、请求体、认证）；历史从未保存过字节的文件则单独列为「需要重新选择文件」。
...
- 历史列表里，网络截断用一个图标标出；把鼠标停在它上面会显示完整说明，读屏器读到它时念的也是同一句，完整的一句话在响应面板里。
- 侧栏被拖窄时，历史行只会少显示这三样：响应正文预览、耗时、URL 的字符数；有空间画出省略号时，这三样都以省略号收尾。在侧栏可拖动的整个范围内，方法名、状态码与四个操作按钮都在。它不会换行，也不会让两段文字叠在一起——只有方法名例外：它占固定的一格，本应用写得出的方法名都放得下（最长 `OPTIONS`）；历史文件若被手工改过、方法名比 `OPTIONS` 还长，多出来的字会画到 URL 上，这是有意不修的。把窗口本身缩到很窄（远小于 1024）时，行动按钮会从右向左被裁掉——那个宽度不在支持范围内。侧栏宽度不会被记住，每次启动都回到默认宽度。
- 环境面板的变量表里每个变量占两行：上一行是变量名和两个按钮（把这个值标记为密钥、删除这一行），下一行是值——因此这两个按钮都完整显示、不需要任何滚动就能点到。唯一的例外是侧栏被压到窄得列表区出现横向滚动条时：那时两个按钮仍然能滚到，不会像以前那样没有任何办法碰到。代价是行变高了：同样高的列表里，一屏能看到的变量数大约少一半。
- 侧栏被拖窄时，环境面板里变量名和值这两个输入框一起变窄、只是显示得少；它不会让两个控件叠在一起，也不会让任何文字画到它自己的框外。
...
- 响应面板的「复制」与「下载」不是那个出口。它们给出的是 ApiSolo 手里已经持有的那份正文，而这与屏幕上那份不是一回事：显示截断、JSON 格式化、树形视图改变的都是你看到的内容，不是这两个按钮交给你的内容。它们也不会回头去取任何没有收到的内容。只有当正文直接来自网络、且这次读取跑到了末尾，面板才称其为完整；其余情况它会写明你拿到的是哪一种更弱的东西，而更弱的东西有两种。被网络上限截断的正文，按「已接收的部分」交付。从历史回放的响应，按「历史存档快照」交付，ApiSolo 无法为它的完整性背书——历史在保存时会把过长的正文截短，而且不会留下任何标志说它这么做过。
- ApiSolo 判定为二进制的响应体两个按钮都不提供：那些字节在到达界面之前已被一条说明替换，那里没有可交付的东西。在 ApiSolo 开始记录这个区分之前写下的历史行会被还原成文本，因此它们是有按钮的——按钮交出的是那条说明而不是字节，面板也不会称其为完整。
