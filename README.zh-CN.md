...
- 侧栏被拖窄时，历史行只会少显示这三样：响应正文预览、耗时、URL 的字符数，都以省略号收尾。在侧栏可拖动的整个范围内，方法名、状态码与四个操作按钮都在。它不会换行，也不会让两段文字叠在一起——只有方法名例外：它占固定的一格，本应用写得出的方法名都放得下（最长 `OPTIONS`）；历史文件若被手工改过、方法名比 `OPTIONS` 还长，多出来的字会画到 URL 上，这是有意不修的。把窗口本身缩到很窄（远小于 1024）时，行动按钮会从右向左被裁掉——那个宽度不在支持范围内。侧栏宽度不会被记住，每次启动都回到默认宽度。
