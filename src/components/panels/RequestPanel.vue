...
import { clearSentinelPairs } from "../../utils/redaction";
...
const bodyPendingFields = computed(() =>
  pendingFields.value.filter((field) => field.source === "body" || field.source === "form"),
);
...
function applyCurlImport() {
  ...
  try {
    ...
    tabsStore.updateTab(activeTab.value.id, {
      ...
      body: {
        ...parsed.body,
        formData: clearSentinelPairs(parsed.body.formData),
      },
      auth: parsed.auth,
      isDirty: true,
    });
    ...
    <div class="flex-1 min-h-0 overflow-auto p-4">
      ...
      <div v-show="activeSection === 'body'" class="h-full">
        <BodyEditor
          ...
          :pending-fields="bodyPendingFields"
