...
- Unreadable secret metadata: a `.env.secrets.json` that cannot be parsed is renamed to `<name>.env.secrets.corrupt-<timestamp>.json` with its bytes untouched, rather than being overwritten. To recover it by hand, rename it back.
