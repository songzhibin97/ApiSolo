...
- The response panel's Copy and Download buttons are not that escape hatch. They hand over the body ApiSolo already holds, which is not the same thing as what is on screen: the display truncation, the JSON pretty printing and the tree view all change what you are looking at, not what those buttons give you. They never go back for anything that was not received. The panel calls a body complete only when it came straight off the network and the read ran to the end; in every other case it says which weaker thing you are getting, and there are two. A body the network cap cut short is offered as the received portion only. A response replayed from history is offered as a stored snapshot whose completeness ApiSolo cannot vouch for, because history shortens long bodies when it saves them and records no flag saying it did.
- A response body ApiSolo knows to be binary offers neither button: those bytes are replaced with a note before they reach the interface, so there is nothing there to hand over. History rows written before ApiSolo recorded that distinction are restored as text and do get the buttons — what those hand over is the note, not the bytes, and the panel does not call it complete.
