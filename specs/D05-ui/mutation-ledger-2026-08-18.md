...
| `UrlBar.vue`：`detectTemplateVariables(draft.value)` → `(props.url)` | 变量提示按回灌串而非草稿计算。**探针 B 实测**：草稿里的变量所在行被渲染串丢掉时（例：键为纯空白的参数行，§3 会丢掉它），提示条由 `true` 变 `false`——**用户写了变量，界面说没有变量** | 要抓它得断言提示条的存在性；提示条没有稳定选择器，加 `data-testid` 属于为可测性动生产代码，本轮未做 | 触发面窄，**未安排检查点**，如实登记 |

第一条的代价必须说清楚：**它能让本切片的核心缺陷完整复发，而全套自动化测试不会红。** 4.5-f 是它当前唯一的防线。

第二条原先我写的是「影响面小（两串的变量集合通常相同）」——那是推断。探针 B 跑完之后应当改成：**两串的变量集合在绝大多数输入下相同，但存在一类输入使它们不同，且不同的方向是「有变量却不提示」**，也就是踩在「UI 不许说谎」上，只是触发面窄。**结论从「小」改成「窄」，这两个词不是一回事。**
...
- URL 栏「输入框绑到草稿而不是 prop」这一条接线**没有任何自动化覆盖**，只有真机检查点 4.5-f。探针实测：补丁生效时输入框把用户打的 `%20` 改写成 `+`，**本切片的核心缺陷完整复发**。
